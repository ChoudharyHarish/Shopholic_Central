...
import { createOrder, getCurrOrder, getOrders, modifyCart } from "../redux/userSlice";
...
export default function Success() {
  ...
  const {currOrderId,orderDate,currOrder,orderFee,shippingFee:tranportFee} = useSelector((state) => state.user);

  const createOrderRef = useRef(false);

  const { cart:userCart } = useSelector((state) => state.user);
  ...
  const subTotal = Number(JSON.parse(localStorage.getItem("totalPrice")));
  const shippingFee = 40;

  // const result;
  useEffect(() => {

    if(createOrderRef.current === false){
      dispatch(createOrder({ cart, subTotal, shippingFee }));
      dispatch(modifyCart({ products:[], price:0 }));    
      createOrderRef.current = true; 
    }
    else {
      dispatch(getCurrOrder(JSON.parse(localStorage.getItem('orderId'))))
    }
    //  Two ways to get order details simply use the function of redux or we can also use this payload;

  },[]);

  return (
    <div className="py-28 px-16 md:px-12  2xl:px-20 2xl:container 2xl:mx-auto">
